...
public class ProductController {
	...
	public void productupload(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			try {
				response.getWriter().print("<html><head><script src=\"/library/js/jquery-1.8.3.min.js\"></script><script>$('#alertError', top.document).val('"+PortalTools.getInstance().getRespError(e).getMessageError().toString()+"');$('#alertError', top.document).click()</script></head><body></body></html>");
